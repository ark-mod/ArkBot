...
{
    public class Structures2Controller : ApiController
    {
        ...
        public Structures2Controller(ArkContextManager contextManager)
        {
            _contextManager = contextManager;
        }

        public Structures2ViewModel Get(string id)
        {
            var context = _contextManager.GetServer(id);
            if (context == null) return null;

            var result = new Structures2ViewModel
            {
                    ...
                    .SelectMany(x =>
                {
                    ...
                    var owner = owners.GetOrAdd(arkOwnerId, (key) =>
                    {
                        return new StructureOwnerViewModel(new Lazy<int>(() => ids.AddOrUpdate("ownerId", 0, (key2, value) => value + 1)))
                        {
                            ...
                            Type = first.TargetingTeam.HasValue ? "tribe" : "player",
                            LastActiveTime = (first.TargetingTeam.HasValue ? context.Tribes.FirstOrDefault(y => y.Id == first.TargetingTeam.Value)?.LastActiveTime : context.Players.FirstOrDefault(y => y.Id == first.OwningPlayerId)?.SavedAt)
                        };
                    });
                    ...
                    do
                    {
                        ...
                        var structureGroups = area.Structures.GroupBy(y => y.Item1).Select(y => new Structure2ViewModel
                        {
                            TypeId = y.Key,
                            Count = y.Count()
