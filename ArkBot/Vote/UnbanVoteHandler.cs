...
namespace ArkBot.Vote
{
    public class UnbanVoteHandler : IVoteHandler
    {
        ...
        public static async Task<InitiateVoteResult> Initiate(Channel channel, IArkContext context, IConfig config, IEfDatabaseContext db, ulong userId, string identifier, DateTime when, string reason, string targetRaw)
        {
        ...
        public async Task<VoteStateChangeResult> VoteFinished(IConfig config, IConstants constants, IEfDatabaseContext db)
        {
            ...
            if (_vote.Result == VoteResult.Passed)
            {
                ...
                await CommandHelper.SendRconCommand(config, $"unbanplayer {_vote.SteamId}");
            }
