...
{
    public class BanVoteHandler : IVoteHandler
    {
        ...
        public static async Task<InitiateVoteResult> Initiate(Channel channel, IArkContext context, IConfig config, IEfDatabaseContext db, ulong userId, string identifier, DateTime when, string reason, string targetRaw, int durationInHours)
        {
            ...
            var targets = m.Success ?
                ...
                : context.Players.Where(x => (x.Name != null && x.Name.Equals(targetRaw, StringComparison.OrdinalIgnoreCase)) || (x.PlayerName != null && x.PlayerName.Equals(targetRaw, StringComparison.OrdinalIgnoreCase))).ToArray();
            if (targets.Length == 0)
            {
        ...
        public async Task<VoteStateChangeResult> VoteFinished(IConfig config, IConstants constants, IEfDatabaseContext db)
        {
            ...
            if (_vote.Result == VoteResult.Passed)
            {
                ...
                await CommandHelper.SendRconCommand(config, $"banplayer {_vote.SteamId}");
            }
