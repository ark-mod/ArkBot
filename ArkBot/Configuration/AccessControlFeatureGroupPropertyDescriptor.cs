...
{
    public class AccessControlFeatureGroupPropertyDescriptor<T, T2> : PropertyDescriptor
    {
        private readonly IDictionary<T, T2> _owner;
        private readonly T _index;
        private readonly int _order;

        public AccessControlFeatureGroupPropertyDescriptor(IDictionary<T, T2> owner, T index, int order) : base("[" + index + "]", null)
        {
            _owner = owner;
            _index = index;
            _order = order;
